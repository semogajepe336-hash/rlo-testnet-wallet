...
const mkAta:any={programId:ATAP,data:new Uint8Array([1]),accounts:[A(kp.publicKey,true,true),A(ataOf(kp.publicKey),true),A(kp.publicKey,false),A(TMINT,false),A(SYS,false),A(TK22,false)]};
 ...
 if(res.executed===true&&!res.err){setSwapStatus("Swap successful.");setSwapAmt("");await refresh(kp.publicKey);await loadTest(kp.publicKey);await loadLiq();await loadHistory()}else{setSwapStatus("Swap failed on-chain.")}
